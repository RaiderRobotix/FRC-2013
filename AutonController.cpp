...
void AutonController::WayneCokeley() {
	...
	if (m_step == 0) {
		...
		m_turnComplete = drivebase->Turn(25, 5.0, 0.55);
		if(m_turnComplete) {
			m_turnComplete = false;
			drivebase->ResetEncoders();
			drivebase->ResetGyro();
		...
		m_turnComplete = drivebase->Turn(50, 5.0, 0.55);
		shooter->TiltDown();
		shooter->BucketDown();
		shooter->TurnOff();
		pickup->TurnOn(0.7);
...
void AutonController::ShaunMcNulty() {
	if (m_step == 0) {
		...
		drivebase->SetEncoderPID(0.030, 0.0, 0.0);
		m_driveStraightComplete = drivebase->DriveStraight(24, 2.0, 0.0002);
		shooter->BucketDown();
		shooter->TurnOff();
		if (m_driveStraightComplete) {
			m_driveStraightComplete = false;
		...
		m_driveStraightComplete = drivebase->DriveStraight(130, 2.0, 0.0002, 0.6);
		if (m_driveStraightComplete) {
			m_driveStraightComplete = false;
			drivebase->ResetEncoders();
			drivebase->ResetGyro();
